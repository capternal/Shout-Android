...
import com.shoutin.CustomClasses.CircularNetworkImageView;
...
import com.shoutin.main.ShoutUsersListActivity;
...
public class ProfileScreenActivity extends ActionBarActivity implements OnMapReadyCallback, View.OnClickListener, ConnectivityBroadcastReceiver.ConnectivityReceiverListener, CallWebService.WebserviceResponse, CustomListPopup.CustomListItemClickListener {
    ...
    private TextView objEditTextCity;
    ...
    ArrayList<CustomListModel> arrCustomListModel = new ArrayList<CustomListModel>();
...
//    Button btnLoadShouts;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        System.out.println("BACK SCREEN NAME ON CREATE :" + objSharedPreferences.getString(Constants.PROFILE_BACK_SCREEN_NAME, ""));
        ...
        CustomListPopup objCustomListPopup = new CustomListPopup(ProfileScreenActivity.this);
        CustomListPopup.setCustomListItemClickListener(this);
        ...
        try {
            new CallWebService(Constants.GET_ALL_CITIES, new JSONObject(), ProfileScreenActivity.this, this, true).execute();
    ...
    private void init() {
        ...
        objEditTextCity = (TextView) findViewById(R.id.edt_profile_screen_city);
        ...
        objEditTextCity.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                System.out.println("CITY CLICKED");
                CustomListPopup objCustomListPopup = new CustomListPopup(ProfileScreenActivity.this);
                objCustomListPopup.setArrCustomListModel(arrCustomListModel);
                objCustomListPopup.showPopupList();
            }
        });
...
//                    new SaveProfileAPI().execute(strGender, strDateOfBirth, String.valueOf(currentLatitude), String.valueOf(currentLongitude));
    ...
    public void onWebserviceResponce(String strUrl, String strResult) {
        switch (strUrl) {
            case Constants.GET_ALL_CITIES:
                try {
                    JSONObject objJsonObject = new JSONObject(strResult);
                    if (objJsonObject.getBoolean("result")) {
                        JSONArray objJsonArray = new JSONArray(objJsonObject.getString("city"));
                        for (int index = 0; index < objJsonArray.length(); index++) {
                            arrCustomListModel.add(new CustomListModel(objJsonArray.getJSONObject(index).getInt("id"), objJsonArray.getJSONObject(index).getString("city_name")));
                        }
                    }
                } catch (NullPointerException ne) {
                    ne.printStackTrace();
    ...
    public void onCustomListItemClickEvent(int id, String name) {
        System.out.println("SELECTED CITY DATA ID : " + id + " NAME : " + name);
        objEditTextCity.setText(name);
    }
        ...
        protected void onPostExecute(String s) {
            ...
            try {
                ...
                if (objJsonObject.getBoolean("result")) {
                    ...
                    objEditor.putString(Constants.USER_CITY, objEditTextCity.getText().toString());
                    objEditor.putString(Constants.USER_ADDRESS, objTextViewProfileAdderss.getText().toString());
                    objEditor.commit();
                }
            } catch (NullPointerException ne) {
                ne.printStackTrace();
    ...
    public class GetUserDetailsAPI extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            try {
                ...
                SharedPreferences.Editor objEditor = objSharedPreferences.edit();
                objEditor.putString(Constants.PROFILE_SCREEN_USER_ID, params[0]);
                objEditor.commit();
...
//                    Log.i(TAG, status.getStatusMessage());
    ...
    private void handleBackEvent() {
        ...
        if (Constants.SHOUT_MESSAGE_USER_LIST_SCREEN.equals(strBackScreenName)) {
            ...
            objEditor.commit();
            overridePendingTransition(R.anim.fade_in_activity, R.anim.fade_out_activity);
        } else if (Constants.SHOUT_DEFAULT_SCREEN.equals(strBackScreenName) || Constants.INVITE_FRINEDS_SCREEN.equals(strBackScreenName) || strBackScreenName.equals(Constants.SHOUT_DETAIL_SCREEN)) {
...
//            throw new IllegalArgumentException("Age < 0");
    ...
    private void editableOrNot(boolean status) {
        ...
        if (status) {
            ...
            objEditTextCity.setClickable(status);
        } else {
    ...
    private boolean mayRequestLocationAccess() {
        ...
        if (shouldShowRequestPermissionRationale(ACCESS_FINE_LOCATION)) {
            requestPermissions(new String[]{ACCESS_FINE_LOCATION, WRITE_EXTERNAL_STORAGE, READ_EXTERNAL_STORAGE, CAMERA}, 1);
        } else {
            requestPermissions(new String[]{ACCESS_FINE_LOCATION, WRITE_EXTERNAL_STORAGE, READ_EXTERNAL_STORAGE, CAMERA}, 1);
        }
        return false;
