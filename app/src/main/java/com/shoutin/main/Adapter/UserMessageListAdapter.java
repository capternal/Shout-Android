...
import com.shoutin.main.Model.UserMessageListModel;
...
public class UserMessageListAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (objUserMessageListModel.getItemImage().equals("")) {
            Picasso.with(objContext).load(R.drawable.app_icon_circle).placeholder(R.drawable.default_profile).error(R.drawable.default_profile).transform(new CircleTransform()).into(objViewHolder.objItemImage);
            ...
            if (ConnectivityBroadcastReceiver.isConnected()) {
                Picasso.with(objContext).load(objUserMessageListModel.getItemImage()).placeholder(R.drawable.default_profile).error(R.drawable.default_profile).transform(new CircleTransform()).into(objViewHolder.objItemImage);
            } else {
                Picasso.with(objContext).load(objUserMessageListModel.getItemImage()).placeholder(R.drawable.default_profile).error(R.drawable.default_profile).transform(new CircleTransform()).networkPolicy(NetworkPolicy.OFFLINE).into(objViewHolder.objItemImage);
            }
        }

        objViewHolder.objItemImage.setPadding(Constants.DEFAULT_MESSAGE_BOARD_PADDING,
        ...
        if (objUserMessageListModel.getMessageType().equals("T")) {
            objViewHolder.objTextViewGroupMembers.setText(Utils.decodeString(objUserMessageListModel.getLastMessage()));
        } else if (objUserMessageListModel.getMessageType().equals("I")) {
            objViewHolder.objTextViewGroupMembers.setText(Utils.decodeString("\uD83D\uDCF7 Photo"));
        } else if (objUserMessageListModel.getMessageType().equals("RR") || objUserMessageListModel.getMessageType().equals("LR")) {
            objViewHolder.objTextViewGroupMembers.setText(Utils.decodeString("Request"));
        }
