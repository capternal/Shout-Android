    ...
    public Object instantiateItem(final View container, int position) {
        ...
        if (position == 1) {
            ...
            if (objShoutDefaultListModel.getSHOUT_TYPE().equals("L") || objShoutDefaultListModel.getSHOUT_TYPE().equals("R")) {
                LinearLayout linearLike = (LinearLayout) view.findViewById(R.id.linear_like);
                LinearLayout linearShare = (LinearLayout) view.findViewById(R.id.linear_share);
                LinearLayout linearComment = (LinearLayout) view.findViewById(R.id.linear_comment);
...
//            txtComments.setText(objShoutDefaultListModel.getCOMMENT_COUNT() + " Comments\n" + objShoutDefaultListModel.getENGAGE_COUNT() + " Engaging");
                ...
                if (objShoutDefaultListModel.getSHOUT_IMAGE().equals("")) {
                    ...
                    if (ConnectivityBroadcastReceiver.isConnected()) {
                        Picasso.with(objActivity).load(objShoutDefaultListModel.getSHOUT_IMAGE()).networkPolicy(NetworkPolicy.NO_CACHE).networkPolicy(NetworkPolicy.NO_STORE).error(R.drawable.app_icon).noFade().transform(new BitmapBorderTransformation(0, 15, Color.RED)).noFade().into(shoutImage);
                    } else {
                        Picasso.with(objActivity).load(objShoutDefaultListModel.getSHOUT_IMAGE()).networkPolicy(NetworkPolicy.NO_CACHE).networkPolicy(NetworkPolicy.NO_STORE).error(R.drawable.app_icon).noFade().transform(new BitmapBorderTransformation(0, 15, Color.RED)).noFade().networkPolicy(NetworkPolicy.OFFLINE).into(shoutImage);
                    }
                }
                try {
                    System.out.println("PROFILE IMAGE URL : " + objShoutDefaultListModel.getPROFILE_IMAGE_URL());
                    if (ConnectivityBroadcastReceiver.isConnected()) {
                        Picasso.with(objActivity).load(objShoutDefaultListModel.getPROFILE_IMAGE_URL()).networkPolicy(NetworkPolicy.NO_CACHE).networkPolicy(NetworkPolicy.NO_STORE).placeholder(R.drawable.default_profile).error(R.drawable.default_profile).noFade().transform(new CircleTransform()).into(objProfileImage);
                    } else {
                        Picasso.with(objActivity).load(objShoutDefaultListModel.getPROFILE_IMAGE_URL()).networkPolicy(NetworkPolicy.NO_CACHE).networkPolicy(NetworkPolicy.NO_STORE).placeholder(R.drawable.default_profile).error(R.drawable.default_profile).noFade().transform(new CircleTransform()).networkPolicy(NetworkPolicy.OFFLINE).into(objProfileImage);
                    }
                ...
                System.out.println("LIKE COUNT : " + Integer.parseInt(objShoutDefaultListModel.getLIKE_COUNT()));
                ...
                TextView objTextViewBlogTitle = (TextView) view.findViewById(R.id.txt_shout_in_admin_title);
                TextView objTextViewBlogLabel = (TextView) view.findViewById(R.id.txt_shout_in_admin_label);
                System.out.println("SHOUT BLOG IMAGE URL : " + Constants.HTTP_URL + objShoutDefaultListModel.getSHOUT_IMAGE());
                objTextViewBlogTitle.setText(objShoutDefaultListModel.getTITLE());
                objTextViewBlogLabel.setText(objShoutDefaultListModel.getDESCRIPTION());
                if (objShoutDefaultListModel.getSHOUT_IMAGE().isEmpty()) {
                    ...
                    Picasso.with(objActivity).load(Constants.HTTP_URL + objShoutDefaultListModel.getSHOUT_IMAGE()).transform(new BitmapBorderTransformation(0, 15, objActivity.getResources().getColor(R.color.red_background_color))).into(objBlogBackgroundImageView);
//                    .transform(new BitmapBorderTransformation(0, 15, objActivity.getResources().getColor(R.color.red_background_color)))
                }
    ...
    @Override
    public void onClick(View v) {
        ((ShoutDefaultActivity) objActivity).hideShowEditMapLayout(false);
...
//                    ShoutDefaultActivity.openCloseSearchBar(false, objActivity);
                    ...
                    String strShoutShareMessage = Constants.SHOUT_SHARE_LINK + arrShoutDefaultListModel.get(intPosition).getSHOUT_SHARE_LINK();
...
//                      share.setPackage("com.shout.shout_test");
                    objActivity.startActivity(Intent.createChooser(share, objActivity.getResources().getString(R.string.app_name)));
                }
                break;
                ...
                if (objJsonObject.getString("result").equals("true")) {
                    ...
                    if (strIsHide.equals("0")) {
                        ...
                        Answers.getInstance().logCustom(new CustomEvent(Constants.EVENT_SHOUT_HIDE)
                                ...
                                .putCustomAttribute("Shout Id", objShoutDefaultListModel.getSHOUT_ID())
                                .putCustomAttribute("Position", "Shout Board")
...
//                    ((ShoutDefaultActivity) objActivity).refreshList();
                    ((ShoutDefaultActivity) objActivity).refreshShoutList(objShoutDefaultListModel.getSHOUT_ID());
                   /* System.out.println("BEFORE PASS ARRAY SIZE:" + arrShoutDefaultListModel.size());
                    arrShoutDefaultListModel.remove(intPosition);
                    System.out.println("AFTER PASS ARRAY SIZE:" + arrShoutDefaultListModel.size());
